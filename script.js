...
async function getIPDetails() {
    ...
    let response = await fetch(`https://ipinfo.io/${currentIp}?token=26e0c8b6c4b29e`);
...
// }
function updateMoreInfo(details) {
    // Get user's timezone from IP details
    const userTimeZone = details.Timezone;
    
    // Get current date and time in user's timezone
    const currentTime = new Date().toLocaleString("en-US", { timeZone: userTimeZone });
    
    // Display timezone and date/time
    let timeZoneElement = document.getElementById("timeZone");
    timeZoneElement.innerText += userTimeZone;

    let dateTimeElement = document.getElementById("dateTime");
    dateTimeElement.innerText += currentTime;

    // Other details
    let pin = document.getElementById("pinCode");
    pin.innerText += details.PostOffice[0].PINCode;
